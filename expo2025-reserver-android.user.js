...
// @grant        none
// @updateURL    https://github.com/expo2025-auto/expo2025-tm-autobooker/raw/refs/heads/main/expo2025-reserver-android.user.js
// @downloadURL  https://github.com/expo2025-auto/expo2025-tm-autobooker/raw/refs/heads/main/expo2025-reserver-android.user.js
...
const ANDROID_READY_WAIT_TIMEOUT_MS=15000;
const ANDROID_READY_RETRY_DELAY_MS=1200;
let calendarReadyObserved=false;
let calendarReadyPromise=null;
let pendingReloadRequest=false;
function markCalendarReadyObserved(){calendarReadyObserved=true;}
function waitUntilCalendarObserved(timeout=ANDROID_READY_WAIT_TIMEOUT_MS){
  if(calendarReadyObserved)return Promise.resolve(true);
  if(calendarReadyPromise)return calendarReadyPromise;
  calendarReadyPromise=(async()=>{
    try{
      const ok=await waitCalendarReady(timeout);
      if(ok)calendarReadyObserved=true;
      return !!ok;
    }finally{
...
function requestReloadAfterReady(){
  if(pendingReloadRequest)return;
  pendingReloadRequest=true;
  const stillActive=()=>state.keepAlive||state.r;
  const attempt=()=>{
    waitUntilCalendarObserved().then(ok=>{
      if(!stillActive()){pendingReloadRequest=false;return;}
      if(ok){
        ...
        },ANDROID_READY_RETRY_DELAY_MS);
      }
    }).catch(()=>{
      pendingReloadRequest=false;
      setTimeout(()=>{if(stillActive())requestReloadAfterReady();},ANDROID_READY_RETRY_DELAY_MS);
    });
  };
  attempt();
}
...
function scheduleKeepAliveReload(){
  ...
  KeepAliveTimer=setTimeout(()=>{if(!state.keepAlive)return;KeepAliveNextAt=null;setUIStatus('ログイン維持リロード実行');requestReloadAfterReady();},KEEP_ALIVE_INTERVAL_MS);
}
...
async function waitCalendarReady(timeout=5000){
  ...
  if(settled) markCalendarReadyObserved();
  return settled;
}
...
function scheduleRetryOrNextMinute(){
  ...
  if(sec<53){
    if(state.r){
      ui.setStatus(calendarReadyObserved?'再試行中':'画面準備待機中');
      requestReloadAfterReady();
    }
  }else{
    const d=delayUntilNextMinute_43s();
    ui.setStatus('待機中');
    clearTimeout(Tm);
    Tm=setTimeout(()=>{if(state.r){resetFail();requestReloadAfterReady();}},d);
  }
}
