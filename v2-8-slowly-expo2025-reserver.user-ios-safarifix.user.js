...
function safeReload(){
  try{
    console.log('[NR] safeReload at', new Date().toLocaleTimeString());

    // Safari/iOS: バックグラウンド時の不安定化を避ける
    if (document.visibilityState === 'hidden'){
      console.warn('[NR] Reload postponed: page not visible');
    ...
    try{
      const backup={
        conf: localStorage.getItem(CONF_KEY),
        st: sessionStorage.getItem(STATE_KEY),
        fr: sessionStorage.getItem('nr_forced_reload_v1')
      };
      sessionStorage.setItem('__nr_backup', JSON.stringify(backup));
    }catch(_){}
    ...
    console.warn('[NR] safeReload fallback', e);
    try{
...
const RENDER_READY_CHECK_INTERVAL=150;
const RENDER_READY_EXTRA_FRAMES=2;
const waitMs=ms=>new Promise(res=>setTimeout(res,ms));
const nextFrame=()=>new Promise(res=>requestAnimationFrame(()=>res()));
const RELOAD_DELAY_OFFSET_MS=250;
let pendingReloadTimer=null;
function queueSafeReload(delay=0){
  ...
  if(delay>0){
    pendingReloadTimer=setTimeout(run,delay);
  }else{
    run();
  }
}
async function waitFrames(count){for(let i=0;i<count;i){try{await nextFrame()}catch{await waitMs(16)}}}
function renderReady(){
  try{
    const main=document.querySelector('#__next main');
    if(main&&vis(main))return true;
    const calendar=document.querySelector('div[class^="style_main__calendar__"]');
    if(calendar&&vis(calendar))return true;
...
function scheduleReloadAfterRender(delay,{requireActive=true,resetFailCount=true}={}){
  try{clearTimeout(Tm)}catch{}
  const run=()=>{reloadAfterRender({requireActive,resetFailCount})};
  if(!delay){run();return;}
...
function scheduleKeepAliveReload(){
  ...
  KeepAliveTimer=setTimeout(()=>{if(!state.keepAlive)return;KeepAliveNextAt=null;setUIStatus('ログイン維持リロード実行');reloadAfterRender({requireActive:false,resetFailCount:false});},KEEP_ALIVE_INTERVAL_MS);
}
...
function robustReload(){
  ...
  if (isSafariBrowser()){
    console.log('[NR] robustReload (Safari) → single safeReload');
    // Safari: 多重リロードやwindow.stopは使わない
    try{ safeReload(); }catch(_){}
    setTimeout(()=>{ if(Date.now() - __reloadStamp > 1400){ __reloading = false; } }, 1600);
    return;
  }

  // 他ブラウザ: 既存の戦略を維持（window.stop  queueSafeReload）
  try{ window.stop(); }catch(_){}
  queueSafeReload(RELOAD_DELAY_OFFSET_MS);
  setTimeout(()=>{
    if(Date.now() - __reloadStamp > (1900  RELOAD_DELAY_OFFSET_MS)){
      __reloading = false;
    }
  }, 2000  RELOAD_DELAY_OFFSET_MS);
}
...
async function showMonthForISO(iso){
  ...
  for(let round=0; round<3; round){
    ...
    await waitUntil(()=>{
      ...
      return null;
    },{timeout:900round*500,attrs:['class','style','aria-hidden','aria-pressed'],root});
...
function scheduleRetryWithinWindow(opts){
  ...
  const SAFETY_MARGIN_MS=RELOAD_DELAY_OFFSET_MS400;
  ...
  delay=Math.min(maxDelay,delayMath.random()*250);
