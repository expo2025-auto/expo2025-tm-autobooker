...
// @grant        none
// @updateURL    https://github.com/expo2025-auto/expo2025-tm-autobooker/raw/refs/heads/main/v2-8-expo2025-reserver-ultrafast.user.js
// @downloadURL  https://github.com/expo2025-auto/expo2025-tm-autobooker/raw/refs/heads/main/v2-8-expo2025-reserver-ultrafast.user.js
...
async function waitFirstEnabledSlot(allowedKeys,timeout=6000){
  ...
  return await waitUntil(getter,{timeout,interval:0,attrs:['class','disabled','aria-disabled','data-disabled']});
}
...
async function waitEnabled(selOrEl,timeout=10000){
  ...
  return await waitUntil(()=>{const e=resEl();return(e&&isEnabled(e))?e:null},{timeout,interval:0,attrs:['class','disabled','aria-disabled','data-disabled']});
}
...
async function waitTypeSelectionPage(timeout=8000){
  ...
  const hit=await waitUntil(()=>isTypeSelectionPage()?true:null,{timeout,interval:0,attrs:['data-message-code','class','style','aria-hidden']});
  return !!hit;
}
async function flowConfirm(targetISO,{targetTimeKey,allowedTimeKeys}={}){
  ...
  if(!b){
    b=await waitUntil(()=>findConfirmBtn(clickedButtons),{timeout:12000,interval:0,attrs:['class','disabled','aria-disabled','data-disabled','aria-hidden']});
  }
  ...
  if(selectedDateISO()!==targetISO)return 'none';
  const nextBtn=await waitUntil(()=>findConfirmBtn(clickedButtons),{timeout:8000,interval:0,attrs:['class','disabled','aria-disabled','data-disabled','aria-hidden']});
  if(nextBtn){
    KC(nextBtn);
...
async function waitOutcome(timeout=12000){
  const result=await waitUntil(()=>{
    ...
    return null;
  },{timeout,interval:0,attrs:['class','style','aria-hidden','data-message-code']});
  return result||'none';
}
