// ==UserScript==
// @name         Expo2025 予約オート（Safari安定化版）
// @namespace    http://tampermonkey.net/
// @version      2025-10-06-safari-fix
// @description  iPad Safariでのトップページへ戻される現象を抑制。毎分探索/自動予約の現行機能は維持。
// @match        https://ticket.expo2025.or.jp/*
// @run-at       document-idle
// @grant        none
// @downloadURL  https://raw.githubusercontent.com/<your-account>/<your-repo>/main/expo2025-reserver-ios-safarifix.user.js
// @updateURL    https://raw.githubusercontent.com/<your-account>/<your-repo>/main/expo2025-reserver-ios-safarifix.user.js
// ==/UserScript==
...
function robustReload(){
  if (typeof window.__reloading === 'undefined') window.__reloading = false;
  if (typeof getFail === 'function' && typeof setFail === 'function'){
    const n = getFail();
    if (n >= 3) return;
    setFail(n + 1);
  }
  ...
  if (isSafariBrowser()){
    ...
    safeReload();
    setTimeout(()=>{ window.__reloading = false; }, 2000);
  }else{
    try{ window.stop?.(); }catch(_){}
    try{ safeReload(); }catch(_){}
    setTimeout(()=>{ try{ location.replace(location.href); }catch(_){} }, 120);
    setTimeout(()=>{ try{
      const u = new URL(location.href);
      u.searchParams.set('r', Date.now());
      location.assign(u.toString());
    }catch(_){} }, 240);
    setTimeout(()=>{ try{ history.go(0); }catch(_){}} , 360);
    setTimeout(()=>{ try{ location.href = location.href; }catch(_){} }, 480);
    setTimeout(()=>{ if(Date.now() - stamp > 1900) window.__reloading = false; }, 2000);
  }
}
...
async function waitCalendarReady(timeout=5000){
  ...
  const ok = await waitUntil(
    () => document.querySelector('.style_selector_item__9RWJw time[datetime]') || null,
...
function scheduleRetryOrNextMinute(){
  ...
  if(sec < 25 && forcedState.count < 3 && !isSafariBrowser()){
    ui.setStatus('再試行中');
    clearTimeout(Tm);
